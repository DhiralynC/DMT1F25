...
ANSWER: It's wrong when the new name (*y*) occurs **free in the body** of the
abstraction, because renaming would **capture** those free occurences and change
the meaning.
@@@ -/
...
-- Answer here
def ifThenElsePoly {a : Type} (b : Bool) (t e : a) : a :=
  match b with
  | true => t
  | false => e
